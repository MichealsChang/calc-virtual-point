...
public class RepairingJob {
	...
	public void dayLeakRepairing() {
		...
		if (baseList.getTotalRows() > 0) {
			...
			LogUtils.debugLog("当日补漏 | 获取MQ连接是否成功：" + conn);
			if (conn) {
				for (TaskIplFailed fail : baseList.getList()) {
					try {
						...
						LogUtils.debugLog("当日补漏 | 计算公式：" + formula);
						...
						LogUtils.debugLog("当日补漏 | 参数个数：" + params.size());
						...
						for (Map.Entry<Integer, Object> en : params.entrySet()) {
							...
							LogUtils.debugLog("当日补漏 | query|" + JSONObject.toJSONString(q) + "|耗时：" + (end - start) + " ms...");
							if (r != null) {
								if (r.hasError()) {
									LogUtils.debugLog("当日补漏 | read from " + r.getHost() + ":" + r.getPort() + " failure!");
									for (TSError error : r.getErrors()) {
										LogUtils.debugLog("日终补漏 | " + error.getMessage());
									}
								} else {
									LogUtils.debugLog("当日补漏 | read from " + r.getHost() + ":" + r.getPort() + " success!");
										...
										if (dps.size() > 0) {
											for (TSDataPoint dp : dps) {
												...
												if (dat.getMinutes() % 15 == 0) {
													...
													LogUtils.debugLog("当日补漏 | " + "tm:" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(dp.getTimestamp()) + ", " + "value:" + dp.getValue());
												}
											}
										} else {
						...
						LogUtils.debugLog("当日补漏  | 将要写入MQ中的消息：" + msg);
//						boolean success = MQClient.send(msg);
//						if (success) {
//							LogUtils.debugLog("当日补漏 | 写入MQ成功...");
//							taskIplFailedService.deleteById(fail.getId());
//							LogUtils.debugLog("当日补漏 | 删除补漏成功的数据...");
//						} else {
//							LogUtils.debugLog("当日补漏 | 写入MQ失败...");
//						}
						...
						LogUtils.debugLog("当日补漏 | +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");

					} catch (Exception e) {
						LogUtils.error("当日补漏 | " + e.getMessage());
						e.printStackTrace();
					}
				}
			} else {
				LogUtils.debugLog("当日补漏 | 获取MQ连接失败！");
			}
		}
		LogUtils.debugLog("************************* 当日补漏结束 !   *************************");
	}
